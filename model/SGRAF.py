...
class EncoderImage(nn.Module):
    ...
    def __init__(self, img_dim, embed_size, no_imgnorm=False):
        ...
        self.fc2 = nn.Linear(img_dim, embed_size)
    ...
    def forward(self, images):
        ...
        img_emb1 = self.fc1(images)
        img_emb2 = self.fc2(images)

        # normalize in the joint embedding space
        if not self.no_imgnorm:
            img_emb1 = l2norm(img_emb1, dim=-1)
            img_emb2 = l2norm(img_emb2, dim=-1)

        return img_emb1, img_emb2
...
class EncoderSimilarity(nn.Module):
    ...
    # Modifications by extending UGNCL
...
class SGRAF(object):
    ...
    def __init__(self, opt):
        self.tau = opt.tau  # Modifications by extending UGNCL
    ...
    def forward_emb(self, images, captions, lengths):
        ...
        img_embs1, img_embs2 = self.img_enc(images)
        cap_embs = self.txt_enc(captions, lengths)
        return img_embs1, img_embs2, cap_embs, lengths

    # Modifications by extending UGNCL
    def forward_sim(self, img_embs, cap_embs, cap_lens, mode='sim', evi_mode="soft"):
        ...
        if mode == 'sim':
            ...
            if evi_mode == "warmup":
                sims, evidences, sims_tanh = torch.sigmoid(raw_sims), torch.exp(
                    self.softplus(raw_sims)/self.tau), torch.tanh(raw_sims)
            else:
                sims, evidences, sims_tanh = torch.sigmoid(raw_sims), torch.exp(self.softplus(raw_sims) / self.tau), torch.tanh(raw_sims)
    ...
    def train_emb(self, images, captions, lengths, ids=None, *args):
        ...
        img_embs1, img_embs2, cap_embs, cap_lens = self.forward_emb(images, captions, lengths)
